    ...
    public void payCallbackHandler(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
            ...
            if (status == null || amountParam == null || txnRef == null) {
                response.sendRedirect(frontendUrl + "/invoices/failure?error=missing_parameters");
                return;
            }
            ...
            if (status.equals("00")) {
                ...
                response.sendRedirect(frontendUrl + "/invoices/success?invoiceId=" + invoiceId + "&amount=" + amount);
            } else {
                // redirect to frontend failure page
                response.sendRedirect(frontendUrl + "/invoices/failure?invoiceId=" + invoiceId + "&status=" + status);
            }
        } catch (Exception e) {
            // Log the error and redirect to failure page
            e.printStackTrace();
            response.sendRedirect(frontendUrl + "/invoices/failure?error=" + e.getMessage());
        }
    }
}
